...
def benchmark_execution(
    ...
    #         print("test1")
    #         print(vm.compile(mod, target, params=params))
    #         print("test2")
    ...
    def get_vm_output_with_profiling(
        mod, data, params, target, dev, dtype="float32", number=2, repeat=20
    ):
        with tvm.transform.PassContext(opt_level=4):
  
            # print(pass_ctx.config["relay.collage.tvm_max_depth"])
            exe = relay.vm.compile(mod, target, params=params)
            vm_profiler = profiler_vm.VirtualMachineProfiler(exe, dev)

        report = vm_profiler.profile(
            data,
            func_name="main",
        ...
        if measure:
            print("Evaluate vm inference cost of {} on {}".format(model, repr(dev)))
            ftimer = vm_profiler.module.time_evaluator("invoke", dev, number=number, repeat=repeat)
            # Measure in millisecond.
            prof_res = np.array(ftimer("main", data).results) * 1000
            print(
                "Mean vm inference time (std dev): %.2f ms (%.2f ms)"
    ...
    for target, dev in testing.enabled_targets():
        ...
        # print(tvm.runtime.Device)
        if "llvm" in target:  # Skip CUDA target
            
        # # print("pass!")
          # tvm_out = get_graph_executor_output(mod, tvm.nd.array(data.astype(dtype)), params, target, dev, dtype)
          ...
          report = get_vm_output_with_profiling(mod, tvm.nd.array(data.astype(dtype)), params, target, dev, dtype)
          print(report)
